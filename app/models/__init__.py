...
class GameSlot(db.Model):
    ...
    @property
    def compensation_score_visible(self) -> bool:
        """
        Компенсационные баллы ФСМ (п.8.6) считаются на дистанции ВСЕГО
        турнира — их величина по конкретной игре "плавает" (пересчитывается
        заново) на каждой новой сыгранной игре турнира вплоть до самой
        последней. Пока турнир не завершён, любое текущее значение —
        промежуточное и может искажать место в рейтинге серии/турнира,
        которая ещё идёт (например, единственная сыгранная игра вечера
        может дать игроку неоправданно большое преимущество). Поэтому в
        total_score (а значит, и во всех местах, что на него опираются —
        общий рейтинг, рейтинг серии/этапа, статистика игрока) компенсация
        учитывается только после того, как турнир целиком помечен
        завершённым — total_score вычисляется на лету, а не хранится,
        так что переключение сразу применяется ко всей истории турнира
        без отдельного пересчёта.
        """
        return bool(self.game and self.game.tournament and self.game.tournament.status == "finished")

    @property
    def total_score(self) -> float:
        compensation = self.compensation_score if self.compensation_score_visible else 0.0
        return round(
            self.base_score + self.bonus_score + self.pu_bonus + compensation, 2
        )
