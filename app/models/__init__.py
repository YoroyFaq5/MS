...
class GameSlot(db.Model):
    ...
    @property
    def compensation_score_final(self) -> bool:
        """
        Компенсационные баллы ФСМ (п.8.6) считаются на дистанции ВСЕГО
        турнира и пересчитываются заново на каждой сыгранной игре турнира
        вплоть до самой последней — то есть текущее значение всегда
        актуально относительно уже сыгранных игр, но перестаёт быть
        окончательным, пока турнир не завершён (доиграются ещё игры —
        число может измениться). Используется только для пометки в
        интерфейсе ("предварительно"), а не для того, чтобы прятать
        баллы из total_score — их видно и учитывают в рейтинге всегда,
        см. total_score ниже.
        """
