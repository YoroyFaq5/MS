...
# Минимум игр в сезоне, чтобы претендовать на подиум (топ-3).
# Игрок с меньшим числом игр остаётся в общей таблице на своём
# фактическом месте по очкам, но НЕ занимает призовую позицию —
# она уходит следующему квалифицированному игроку.
MIN_GAMES_FOR_TOP3 = 8
...
class SeasonRatingEntry:
    ...
    rank: int = 0
    podium_rank: Optional[int] = None  # 1/2/3 только если games_played >= MIN_GAMES_FOR_TOP3
...
class SeasonRatingEngine:
    ...
    def compute_season_ratings(season_id: int) -> List[SeasonRatingEntry]:
        ...
        # Подиум (топ-3): только среди игроков с MIN_GAMES_FOR_TOP3+ играми.
        # Список уже отсортирован по season_rating — берём первых трёх
        # квалифицированных подряд, не трогая rank остальных.
        podium_place = 1
        for e in entries:
            if podium_place > 3:
                break
            if e.games_played >= MIN_GAMES_FOR_TOP3:
                e.podium_rank = podium_place
                podium_place += 1
